...
/*
 ...
 -1----------无效
 */
int Get_Breakpoint_Available(BreakPoint_ST *breakpoint, char *url,
		char *file_crc)
{

	//判断断点是否有效,后续加入文件校验码
	if ((memcmp(breakpoint->url, url, strlen(url)) == 0)
...
int Get_Breakpoint(char *url, char *filename, char *file_crc)
{
	...
	//断点文件名 filename+bp
	sprintf(filename_bp, "%s", filename);
...
/*
 ...
 >0--------有断点,已下载文件大小
 */
int Save_Breakpoint(char *url, char *filename, int download_size,
		char *file_crc)
{
	int fd;
...
int Save_File(char *filebuf, int filelength, char *filename)
{
	int fd;
	/* 创建目的文件追加写 */
	if ((fd = open(filename, O_WRONLY | O_CREAT | O_APPEND, S_IRUSR | S_IWUSR))
			== -1)
	{
...
/*
 ...
 -1----------失败
 */
int HTTP_GetFile(int sockfd, char *path, int filelength, int download_size,
		char *filebuf)
{
	int count;
	char range[32];
	...
	count = (filelength % MAX_RECV_SIZE) ?
			(filelength / MAX_RECV_SIZE + 1) : (filelength / MAX_RECV_SIZE);
	...
	for (i = download_index; i < count; i++)
	{
		...
		if ((i == (count - 1)) && (filelength % MAX_RECV_SIZE))
			...
			sprintf(range, "%d-%d", i * MAX_RECV_SIZE,
					(i + 1) * MAX_RECV_SIZE - 1);
